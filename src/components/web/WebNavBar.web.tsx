...
function NavLink({
  ...
  const linkStyle = StyleSheet.flatten([styles.navLink, active && styles.navLinkActive]);
  const linkTextStyle = StyleSheet.flatten([styles.navLinkText, active && styles.navLinkTextActive]);
  return (
          ...
          {!isMobile ? (
            <>
              ...
              <View style={styles.actions}>
                ...
                >
                  <IconSymbol name={isDark ? 'sun.max.fill' : 'moon.fill'} size={18} color={theme.colors.text as any} />
                </Pressable>
            ...
            <View style={styles.mobileActions}>
              ...
              >
                <IconSymbol name={menuOpen ? 'xmark' : 'ellipsis'} size={18} color={theme.colors.text as any} />
              </Pressable>
            </View>
          )}
        </View>
        ...
        {isMobile && menuOpen ? (
          <View style={styles.mobilePanel}>
            <View style={styles.mobileNav}>
              <NavLink href="/about" label="About" onPress={() => setMenuOpen(false)} />
              <NavLink href="/features" label="Features" onPress={() => setMenuOpen(false)} />
              <NavLink href="/pricing" label="Pricing" onPress={() => setMenuOpen(false)} />
              <NavLink href="/contact" label="Contact" onPress={() => setMenuOpen(false)} />
            </View>

            <View style={styles.mobileCtas}>
              <Link href="/auth/login" asChild>
                <Pressable style={StyleSheet.flatten([styles.btn, styles.btnGhost])} onPress={() => setMenuOpen(false)}>
                  <Text style={styles.btnGhostText}>Log in</Text>
                </Pressable>
              </Link>
              <Link href="/auth/signup" asChild>
                <Pressable style={StyleSheet.flatten([styles.btn, styles.btnPrimary])} onPress={() => setMenuOpen(false)}>
                  <Text style={styles.btnPrimaryText}>Get started</Text>
                </Pressable>
              </Link>
            </View>
...
function createStyles(theme: WebThemeShape) {
  return StyleSheet.create({
    ...
    btnPrimary: {
      ...
      borderWidth: 1,
      borderColor:
        theme.colors.bg === '#FFFFFF' ? 'rgba(16, 24, 40, 0.10)' : 'rgba(234, 240, 255, 0.10)',
